...
import { useAddWithdrawPinMutation, useChangePasswordMutation } from "@/redux/features/user/userApi";
...
const AccountSettingSecurity = () => {
    ...
    const handleWithdrawPinSubmit = async () => {
        if (otp.length !== 4) {
            ...
            await createWithdrawPin(submittedData).unwrap().then((res: ResponseErrorType) => {
                if (!res.data?.success) {
                    toast.error(res?.data?.message || "Something went wrong");
                }
                toast.success("withdrawalPin are Created successfully!", { toastId: 1 });
    ...
    const onSubmit: SubmitHandler<FormData> = async (data) => {
        ...
        if (data.confirmPassword !== data.password) {
            ...
            await updatePassword(submittedData).unwrap().then((res: ResponseErrorType) => {
                if (!res.data?.success) {
                    toast.error(res?.data?.message || "Something went wrong");
                }
                ...
                <div className='flex justify-between'>
                    ...
                    <div className='w-[37%] space-y-3'>
                        ...
                        <div className='flex items-center justify-end'>
                            <button type="button" onClick={handleWithdrawPinSubmit} className="appBtn">Create Pin</button>
                        </div>
